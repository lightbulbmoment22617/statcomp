...
When considering the way the two functions work these results are not surprising. `optimHess` takes as a paramter a function to optimise and carries this out numerically, which for non-trivial functions is sure to take time. `myhessian` on the other hand performs a few simple calculations directly with some input parameters. The algorithm itself is $\mathcal{O}(\Psi') + \mathcal{O}(1)$ which means its runtime is not dependent on how complex the negated log-likelihood function is. 
